...
st.set_page_config(
    page_title="RepoLens Morning Sheet",
    page_icon="📈",
    layout="wide",
...
def apply_custom_css() -> None:
    """
    Apply restrained institutional dashboard styling.
    """
    st.markdown(
        """
        <style>
...
def read_optional_password() -> str | None:
    """
    Read an optional app password from Streamlit secrets.

    The app remains open locally when no password has been configured.
    """
    try:
...
def require_password() -> None:
    """
    Display a password gate only when APP_PASSWORD is configured.
    """
...
def format_number(
    ...
    """
    Format optional numerical values for dashboard display.
    """
    if pd.isna(
        value
    ):
...
def status_css_class(
    ...
    """
    Map the Morning Sheet status to a display class.
    """
...
def build_rate_chart(
    ...
    """
    Plot €STR and the ECB deposit facility rate.
    """
    figure = go.Figure()
...
def build_policy_spread_chart(
    ...
    """
    Plot the €STR policy spread and its rolling z-score.
    """
...
def build_market_quality_chart(
    ...
    """
    Plot the derived market-quality score and eligible volume.
    """
    figure = make_subplots(
        specs=[
            [
...
def render_sidebar(
    ...
    """
    Render dashboard controls and return the selected history.
    """
    st.sidebar.title(
        "Morning Sheet"
    )
    ...
    st.sidebar.caption(
        "Euro unsecured overnight funding intelligence"
    )
    ...
    st.sidebar.caption(
        "Market-quality scores, regimes and alerts "
        "are RepoLens derived research analytics."
    )

    if read_optional_password() is not None:
        st.sidebar.divider()
...
def render_latest_summary(
    ...
    """
    Render the latest Morning Sheet observation.
    """
    status = str(
        latest[
            "overall_status"
...
def render_history_table(
    ...
    """
    Render a concise historical table.
    """
    st.subheader(
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "overall_status": st.column_config.TextColumn(
                "Status"
            ),
        },
    )
...
def main() -> None:
    """
    Render the RepoLens Streamlit Morning Sheet.
    """
    apply_custom_css()
    require_password()
    ...
    selected_history = render_sidebar(
        morning_sheet
    )
    ...
    with context_column:
        ...
        st.info(
            "The underlying market observations are official ECB data. "
            "Scores, alerts, classifications and regimes are RepoLens "
            "derived research outputs and are not official ECB indicators."
        )
    ...
    st.caption(
        "RepoLens research dashboard • "
        "Official ECB inputs with transparent derived analytics"
    )
